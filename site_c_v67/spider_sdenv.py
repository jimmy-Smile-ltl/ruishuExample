...
原理:
  ...
  2. sdenv 在 Node.js 里用 jsdom 完整加载 412 页面并执行瑞数 VM -> P-cookie
...
"""
    ...
    result = subprocess.run(["node", str(node_path)], capture_output=True,
                            text=True, timeout=30, cwd=str(SCRIPT_DIR))
    p_cookie = result.stdout.strip().split('\n')[-1]
    print(f"  P-cookie: {len(p_cookie)} chars")

    # Step 3: P-cookie 合并进 session (O-cookie 已在 session 里)
    print("[3/3] 合并 O+P cookie...")
    for part in p_cookie.split(";"):
        part = part.strip()
        if "=" in part and not part.startswith(
                ("path=", "expires=", "Secure", "domain=", "Max-Age=", "HttpOnly", "SameSite")):
            k, v = part.split("=", 1)
...
if __name__ == "__main__":
    ...
    print(f"\n结果: {resp.status_code} | {len(resp.text)} bytes | {len(links)} 个文章链接 | 耗时 {elapsed:.1f}s")
    if resp.status_code == 200:
        (SCRIPT_DIR / "site_c_200_sdenv.html").write_text(resp.text, encoding="utf-8")
