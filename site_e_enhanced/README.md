...
站点E 已**弃用瑞数 WAF**：全站返回空 404 + `wzws-ray` 响应头（网神 WAF 标识）。
本目录脚本对站点E 当前不可用，**保留作为瑞数逆向参考示例**；
用于其他瑞数站点时改 `TARGET_URL` / `BASE_URL` / `UA` 即可，用前确认
目标站仍有 412 + `$_ts` 特征。
...
## 可行方案（验证时间：换 WAF 前的实测）

| 方案 | 文件 | 依赖 | 实测结果 |
|------|------|------|---------|
| 1. sdenv jsdom 补环境（无浏览器） | `spider_sdenv.py` | curl_cffi + Node + sdenv | ✅ 343 chars P / 200 |
| 2. jsdom + 同步 flush（sdenv 提速版） | `spider_jsdom_sync.py` + `jsdom_gen.js` | 同上 | ✅ **421 chars P 秒出**（比方案1 快 5-10 倍） |
| 3. CloakBrowser 隐身浏览器 | `spider_cloakbrowser.py` | cloakbrowser | ✅ 浏览器原生 Cookie / 200（~83% 成功率） |
...
- **用前先确认站点仍是瑞数**：412 + `$_ts` 特征；空 404 + `wzws-ray` 头 = 已换网神 WAF。
