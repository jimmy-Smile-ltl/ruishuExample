...
async function main() {
  ...
  const captureInner = args.includes('--capture-inner');  // TEMP-PURE: 内层码插桩 (ans 真相)
  ...
  if (captureInner) {
    try { Object.defineProperty(ctx, '__ansDump', { value: [], writable: true, configurable: true, enumerable: false }); } catch (e) {}
    // (2026-09-05: concat/push hook 已移除 — __seen[this] 数组键强制转换 = V8 崩溃源; 组装协议已实锤无需再捕)
    try {
      vmMod.runInContext(
        '(function(){})();', ctx);
      log('[INNER-INJ] hooks cleared');
    } catch (e) { log('[INNER-INJ] hook fail: ' + e.message); }
    try {
      vmMod.runInContext(
        '(function(){var __deepStr=function(v,seen){'
        + 'if(v===null||typeof v!=="object"){if(typeof v==="string")return JSON.stringify(v.slice(0,400));return String(v);}'
        + 'if(!seen)seen=[];if(seen.indexOf(v)>=0)return "<circ>";seen.push(v);'
        + 'if(Array.isArray(v)){var a=[];for(var i=0;i<v.length&&i<400;i++)a.push(__deepStr(v[i],seen));var t=a.join(",");seen.pop();return "["+t+(v.length>400?",...":"")+"]";}'
        + 'var out=[],n=0;try{for(var k in v){if(n++>60)break;try{out.push(k+":"+__deepStr(v[k],seen));}catch(_$e1){}}}catch(_$e2){}'
        + 'seen.pop();return "{"+out.join(",")+"}";};'
        + 'globalThis.__deepStr=function(v){return __deepStr(v);};'
        + '})()', ctx);
    } catch (e) {}
  }
  if (fixDate) {
  ...
  const _rawEval = (code) => {
    if (typeof code === 'string' && code.length > 100000) {
      log('[RAW-EVAL] ' + code.length + 'b captureInner=' + process.argv.includes('--capture-inner'));
    }
  // TEMP-PURE: 内层码插桩 — 解释器返回点 dump 任务输出 (真实 ans 真相, 运行时动态提取)
    if (typeof code === 'string' && code.length > 200000 && process.argv.includes('--capture-inner')) {
      try {
        // 收集所有返回点 (两个解释器都插)
        const matches = [];
        let rm;
        const retRe = /(_[\w$]+)\[4\]=1,\1\[5\]=(_[\w$]+)/g;
        while ((rm = retRe.exec(code)) !== null) matches.push({ idx: rm.index, state: rm[1], val: rm[2] });
        log('[INNER-INJ] 返回点 ' + matches.length + ' 处');
        // 加密函数插桩: arguments[2]:1 ... arguments[3]:0 (encrypt 包装, 全局替换两个包装器)
        const encRe = /function (_[\w$]+)\((_[\w$]+),(_[\w$]+),(_[\w$]+),(_[\w$]+)\)\{[^}]*?arguments\[2\]:1[^}]*?arguments\[3\]:0[^}]*?return ([^;}]+)/g;
        const encM = code.match(encRe);
        if (encM) {
          try { require('fs').writeFileSync('encrypt_src.txt', encM[0], 'utf8'); } catch (e) {}
          code = code.replace(encRe, function (full, fn, inp, key, mode, rnd, retExpr) {
            const encOut = ';try{globalThis.__encOut=(globalThis.__encOut||[]);globalThis.__encOut.push(JSON.stringify({fn:\"' + fn + '\",key:String(' + key + '),in:' + inp + '&&String(' + inp + '),mode:' + mode + '}))}catch(_$e7){}';
            return full.replace(/return ([^;}]+)/, () => encOut + 'return ' + retExpr);
          });
          log('[INNER-INJ] 加密函数插桩 ' + encM.length + ' 处');
        } else {
          log('[INNER-INJ] 加密函数特征未找到');
        }
        // encode 单步 dump: X=fn(cfg,block,0,arr1) (getCfg 内 encode 调用形态)
        const encCallRe = /(_[\w$]+)=(_[\w$]+)\((_[\w$]+),(_[\w$]+),0,(_[\w$]+)\)/;
        const encCallM = code.match(encCallRe);
        if (encCallM) {
          const encCallDump = ';try{globalThis.__encStep=(globalThis.__encStep||[]);if(globalThis.__encStep.length<3){globalThis.__encStep.push(JSON.stringify([' + encCallM[4] + '&&' + encCallM[4] + '.slice?(' + encCallM[4] + '.slice(0,16)):' + encCallM[4] + ',' + encCallM[1] + '&&' + encCallM[1] + '.slice?(' + encCallM[1] + '.slice(0,16)):' + encCallM[1] + ']))}}catch(_$e7){}';
          if (code.includes(encCallM[0])) {
            code = code.replace(encCallM[0], encCallM[0].replace(/\)$/, () => ')' + encCallDump));
            log('[INNER-INJ] encode 单步 dump: ' + encCallM[1] + '=' + encCallM[2] + '(' + encCallM[4] + ',0)');
          }
        } else {
          log('[INNER-INJ] encode 调用形态未找到');
        }
        for (let mi = matches.length - 1; mi >= 0; mi--) {
          const { idx, state, val } = matches[mi];
          const old2 = state + '[4]=1,' + state + '[5]=' + val;
          if (code.indexOf(old2) !== idx) continue; // 已被其它替换影响
          const fnStart2 = code.lastIndexOf('function', idx);
          const headSeg = code.slice(fnStart2, idx).slice(0, 1200);
          let taskDump = '0,""';
          const assignM = headSeg.match(/(_[\w$]+)=(_[\w$]+)\.(_[\w$]+)[,;]/);
          if (assignM) {
            taskDump = '(typeof ' + assignM[2] + '!=="undefined"&&' + assignM[2] + '&&' + assignM[2] + '.' + assignM[3] + '?' + assignM[2] + '.' + assignM[3] + '.length:0)' + ',""+' + assignM[2] + '.' + assignM[3] + '.slice(0,400)';
          }
          let inDump = ',"",""';
          const inM = headSeg.match(/(_[\w$]+)=(_[\w$]+)\[2\],(_[\w$]+)=\2\[3\]/);
          if (inM) {
            inDump = ',(function(){var _v=' + inM[1] + ';if(_v===null||typeof _v==="undefined")return "";'
              + 'if(Array.isArray(_v)){var _tl=[];for(var _t=0;_t<_v.length&&_t<80;_t++){var _e=_v[_t];_tl.push(_e===null?"n":typeof _e==="object"?(Array.isArray(_e)?"a"+_e.length:"o"):typeof _e==="string"?("s"+_e.length):"x");}'
              + 'var _full=[];for(var _t2=0;_t2<_v.length&&_t2<20;_t2++){var _ev=_v[_t2];_full.push(_t2+":"+(Array.isArray(_ev)||typeof _ev==="number"?(function(){try{return JSON.stringify(_ev)}catch(_z){return String(_ev).slice(0,300)}})():"<"+typeof _ev+">"));}'
              + 'return "ARR len="+_v.length+" types="+_tl.join(",")+" ELEMS="+_full.join(" | ");}'
              + 'return "TYP "+typeof _v+" "+globalThis.__deepStr(_v).slice(0,800);})()'
              + ',(typeof ' + inM[3] + '!=="undefined"&&' + inM[3] + '!==null?globalThis.__deepStr(' + inM[3] + ').slice(0,600):"")';
          }
          let envTable = '_$g_';
          const envM = code.slice(fnStart2, idx).match(/\((_[\w$]+)=(_[\w$]+)\[\s*\+\+\s*_[\w$]+\],(_[\w$]+)=(_[\w$]+)\)/);
          if (envM) envTable = envM[4];
          // 常量表/字符串表/属性表/子任务表变量
          const bodySeg = code.slice(fnStart2, fnStart2 + 9000);
          const tbls = {};
          let tm;
          {
            const re = /\[_[\w$]+\+\+\s*\]=(_[\w$]+)\[/g;
            let m2, hits = [];
            while ((m2 = re.exec(bodySeg)) !== null) hits.push(m2[1]);
            if (hits.length >= 3) tbls.el = hits[2];
            else if (hits.length >= 2) tbls.el = hits[1];
            else if (hits.length >= 1) tbls.el = hits[0];
          }
          tm = bodySeg.match(/=88,(_[\w$]+)=(_[\w$]+)\[/);
          if (tm) tbls.fw = tm[2];
          tm = bodySeg.match(/_[\w$]+\[(_[\w$]+)\[\s*\+\+\s*_[\w$]+\]\]/);
          if (tm) tbls.aw = tm[1];
          tm = bodySeg.match(/=34,(_[\w$]+)=_[\w$]+\[\s*\+\+\s*_[\w$]+\],_[\w$]+=(_[\w$]+)\[/);
          if (tm) tbls.le = tm[2];
          tm = bodySeg.match(/=(_[\w$]+)\(_[\w$]+\.(_[\w$]+)\[_[\w$]+\],/);
          if (tm) { tbls.iz = tm[1]; tbls.taskprop = tm[2]; }
          log('[INNER-INJ] @' + mi + ' state=' + state + ' taskobj=' + (assignM ? assignM[2] : '?') + ' in2=' + (inM ? inM[1] : '?') + ' el=' + (tbls.el || '?'));
          const tblDump = [];
          if (tbls.el) {
            tblDump.push('el:(function(){var _eo={};var _eok=Object.keys(' + tbls.el + ');for(var _eoi=0;_eoi<_eok.length&&_eoi<200;_eoi++){var _eok2=_eok[_eoi];try{_eo[_eok2]=String(' + tbls.el + '[_eok2]).slice(0,1200)}catch(_eoe){}}return JSON.stringify(_eo)})()');
          }
          for (const k of ['fw', 'aw', 'le']) {
            if (tbls[k]) tblDump.push(k + ':(typeof ' + tbls[k] + '!=="undefined"?(function(){try{return JSON.stringify(' + tbls[k] + ')}catch(_$t1){return String(' + tbls[k] + ').slice(0,300)}})():"undef")');
          }
          const neu2 = state + '[4]=1,' + state + '[5]=' + val +
            ',(function(){try{globalThis.__ansMark=(globalThis.__ansMark||0)+1;'
            + 'if(!globalThis.__envKeys){try{globalThis.__envKeys=Object.keys(' + envTable + ');globalThis.__envSrc={};var _eks=globalThis.__envKeys;for(var _eki=0;_eki<_eks.length&&_eki<400;_eki++){var _ek=_eks[_eki];try{globalThis.__envSrc[_ek]=String(' + envTable + '[_ek]).slice(0,600)}catch(_$ek2){}}}catch(_$ek3){}}'
            + 'if(!globalThis.__tables){try{globalThis.__tables={' + tblDump.join(',') + '};}catch(_$ek4){globalThis.__tablesErr=String(_$ek4)}}'
            + 'globalThis.__ansDump.push(JSON.stringify([' + taskDump + ',' + val + inDump + ']))}catch(_$e){globalThis.__ansErr=(globalThis.__ansErr||0)+1;globalThis.__ansLastErr=String(_$e&&_$e.message)}})()';
          code = code.slice(0, idx) + neu2 + code.slice(idx + old2.length);
          // op3 调用日志: 解释器体内第一处 -=3, (kl + 3 参数)
          if (mi === 0 && assignM && inM) {
            try {
              const bodySeg3 = code.slice(fnStart2, fnStart2 + 6000);
              const op3Idx = bodySeg3.indexOf('-=3,');
              if (op3Idx >= 0) {
                // 提取 kl 变量 (op1 绑定) 和栈变量
                const klVar = envM ? envM[3] : null;
                const stkM = bodySeg3.match(/(_[\w$]+)=(_[\w$]+)\._?[\w$]+\(\)/);
                log('[INNER-INJ] op3 尝试: klVar=' + klVar + ' stkM=' + (stkM ? (stkM[1] + '=' + stkM[2]) : 'null') + ' op3Idx=' + op3Idx);
                if (klVar && stkM) {
                  const call3 = '(function(){try{globalThis.__call3=(globalThis.__call3||[]);if(globalThis.__call3.length<300){globalThis.__call3.push(JSON.stringify([' + klVar + ',' + stkM[1] + '&&' + stkM[1] + '.slice?' + stkM[1] + '.slice(-3):null]))}}catch(_$eC){}})(),';
                  const anchorOp3 = '-=3,';
                  const atIdx = code.indexOf(anchorOp3, fnStart2);
                  if (atIdx >= 0 && atIdx < fnStart2 + 6000) {
                    code = code.slice(0, atIdx + 4) + call3 + code.slice(atIdx + 4);
                    log('[INNER-INJ] op3 调用日志注入: kl=' + klVar + ' stk=' + stkM[1]);
                  }
                  // op5 2参调用日志: 解释器体第一处 -=2,
                  const atIdx5 = code.indexOf('-=2,', fnStart2);
                  if (atIdx5 >= 0 && atIdx5 < fnStart2 + 6000) {
                    const call5 = '(function(){try{globalThis.__call5=(globalThis.__call5||[]);if(globalThis.__call5.length<300){globalThis.__call5.push(JSON.stringify([' + klVar + ',' + stkM[1] + '&&' + stkM[1] + '.slice?' + stkM[1] + '.slice(-2):null]))}}catch(_$eD){}})(),';
                    code = code.slice(0, atIdx5 + 4) + call5 + code.slice(atIdx5 + 4);
                    log('[INNER-INJ] op5 调用日志注入');
                    // op28 (第二处 -=2,): 同样注入
                    const atIdx28 = code.indexOf('-=2,', atIdx5 + 6);
                    if (atIdx28 >= 0 && atIdx28 < fnStart2 + 6000) {
                      const call28 = '(function(){try{globalThis.__call5=(globalThis.__call5||[]);if(globalThis.__call5.length<300){globalThis.__call5.push(JSON.stringify([' + klVar + ',' + stkM[1] + '&&' + stkM[1] + '.slice?' + stkM[1] + '.slice(-2):null]))}}catch(_$eE){}})(),';
                      code = code.slice(0, atIdx28 + 4) + call28 + code.slice(atIdx28 + 4);
                      log('[INNER-INJ] op28 调用日志注入');
                    }
                  }
                }
              }
        ...
        require('fs').writeFileSync('inner_injected.js', code, 'utf-8');
        log('[INNER-INJ] 全部返回点插桩完成');
      } catch (e) { log('[INNER-INJ] err: ' + e.message); }
    }
  ...
  const installNativeEval = () => {
    if (traceTask || debug || captureInner) return; // 调试/捕获模式保留插桩链
    try {
  ...
  if (traceTask || debug || captureInner) { try { win.eval = ctxEval; } catch (e) {} }
  ...
  for (const item of scriptItems) {
    if (item.type === 'external') {
      ...
      try {
        ...
        vmEvaled = true;
        log('VM loaded');
        // TEMP-PURE: dump ctx 全局键 (找内层解释器挂载点)
        try {
          const allKeys = vmMod.runInContext(
            'JSON.stringify(Object.keys(this).filter(function(k){return k.indexOf("_$")===0||k.indexOf("$_")===0||k.indexOf("__")===0;}))', ctx);
          log('[PURE-KEYS] ' + allKeys);
          require('fs').writeFileSync('pure_keys.json', allKeys, 'utf-8');
        } catch (e) { log('[PURE-KEYS] err: ' + e.message); }
        // TEMP-PURE: VM loaded 后 dump 真实 $_ts (与 rs-reverse ts-full.json 同轮对比)
        try {
          const ts = vmMod.runInContext('$_ts', ctx);
          const dump = {
            nsd: ts && ts.nsd, cdLen: ts && ts.cd && ts.cd.length,
            scj: ts && ts.scj, aebi: ts && ts.aebi,
            cp: ts && ts.cp, jf: ts && ts.jf,
          };
          require('fs').writeFileSync('real_ts_dump.json', JSON.stringify(dump), 'utf-8');
          log('[REAL-TS] real_ts_dump.json saved '
            + (dump.aebi ? 'aebi[' + dump.aebi.map(a => Array.isArray(a) ? a.length : typeof a).join(',') + ']' : 'aebi missing'));
        } catch (e) { log('[REAL-TS] err: ' + e.message); }
  ...
  function finish(ck) {
    ...
    // TEMP-PURE: exit 前兜底写 dump
    if (captureInner) {
      try {
        const d = vmMod.runInContext('typeof __ansDump!=="undefined"?__ansDump:[]', ctx);
        if (d && d.length) {
          require('fs').writeFileSync('real_ans_dump.json', JSON.stringify(d), 'utf-8');
          log('[ANS-DUMP] exit 前写出 ' + d.length + ' 条');
        } else {
          log('[ANS-DUMP] exit 前 dump 为空');
          try {
            const t = vmMod.runInContext(
              '(function(){try{globalThis.__ansDump.push("TEST")}catch(e){return "ERR:"+e.message}return "OK:"+(typeof globalThis.__ansDump)+":"+globalThis.__ansDump.length+" mark="+((globalThis.__ansMark||0))+" err="+((globalThis.__ansErr||0))+" lastErr="+((globalThis.__ansLastErr||""))})()', ctx);
            log('[ANS-DUMP] 探针: ' + t);
          } catch (e) { log('[ANS-DUMP] 探针 err: ' + e.message); }
        }
        try {
          const ek = vmMod.runInContext('typeof __envKeys!=="undefined"?__envKeys:[]', ctx);
          const es = vmMod.runInContext('typeof __envSrc!=="undefined"?__envSrc:{}', ctx);
          if (ek && ek.length) {
            require('fs').writeFileSync('env_methods.json', JSON.stringify({ keys: ek, src: es }), 'utf-8');
            log('[ANS-DUMP] env 方法表 ' + ek.length + ' 键已写出');
          }
          const tb = vmMod.runInContext('typeof __tables!=="undefined"?__tables:null', ctx);
          if (tb) {
            require('fs').writeFileSync('vm_tables.json', JSON.stringify(tb), 'utf-8');
            log('[ANS-DUMP] vm 常量表已写出 (el/fw/aw/le/cz)');
          } else {
            log('[ANS-DUMP] __tables 空, err=' + vmMod.runInContext('typeof __tablesErr!=="undefined"?__tablesErr:""', ctx));
          }
          const cl = vmMod.runInContext('typeof __callLog!=="undefined"?__callLog:[]', ctx);
          if (cl && cl.length) {
            require('fs').writeFileSync('call_log.json', JSON.stringify(cl), 'utf-8');
            log('[ANS-DUMP] 调用日志 ' + cl.length + ' 条已写出');
          }
          const el2 = vmMod.runInContext('typeof __encLog!=="undefined"?__encLog:[]', ctx);
          if (el2 && el2.length) {
            require('fs').writeFileSync('enc_log.json', JSON.stringify(el2), 'utf-8');
            log('[ANS-DUMP] 加密调用日志 ' + el2.length + ' 条已写出');
          }
          const cc2 = vmMod.runInContext('typeof __concatLog!=="undefined"?__concatLog:[]', ctx);
          if (cc2 && cc2.length) {
            require('fs').writeFileSync('concat_log.json', JSON.stringify(cc2), 'utf-8');
            log('[ANS-DUMP] concat 日志 ' + cc2.length + ' 条已写出');
          }
          const pp2 = vmMod.runInContext('typeof __pushLog!=="undefined"?__pushLog:[]', ctx);
          if (pp2 && pp2.length) {
            require('fs').writeFileSync('push_log.json', JSON.stringify(pp2), 'utf-8');
            log('[ANS-DUMP] push 日志 ' + pp2.length + ' 条已写出');
          }
          const eo2 = vmMod.runInContext('typeof __encOut!=="undefined"?__encOut:[]', ctx);
          if (eo2 && eo2.length) {
            require('fs').writeFileSync('enc_out.json', JSON.stringify(eo2), 'utf-8');
            log('[ANS-DUMP] 加密输出日志 ' + eo2.length + ' 条已写出');
          }
          const cg = vmMod.runInContext('typeof __cfgLog!=="undefined"?__cfgLog:[]', ctx);
          if (cg && cg.length) {
            require('fs').writeFileSync('cfg_log.json', JSON.stringify(cg), 'utf-8');
            log('[ANS-DUMP] cfg 日志 ' + cg.length + ' 条已写出');
          }
          const cn = vmMod.runInContext('typeof __cfgnumLog!=="undefined"?__cfgnumLog:[]', ctx);
          if (cn && cn.length) {
            require('fs').writeFileSync('cfgnum_log.json', JSON.stringify(cn), 'utf-8');
            log('[ANS-DUMP] cfgnum 日志 ' + cn.length + ' 条已写出');
          }
          const es2 = vmMod.runInContext('typeof __encStep!=="undefined"?__encStep:[]', ctx);
          if (es2 && es2.length) {
            require('fs').writeFileSync('enc_step.json', JSON.stringify(es2), 'utf-8');
            log('[ANS-DUMP] encode 单步日志 ' + es2.length + ' 条已写出');
          }
          const cf = vmMod.runInContext('typeof __cfgnumLog!=="undefined"?__cfgnumLog:[]', ctx);
          if (cf && cf.length) {
            require('fs').writeFileSync('cfgnum_full.json', JSON.stringify(cf), 'utf-8');
            log('[ANS-DUMP] cfgnum 完整表 ' + cf.length + ' 条已写出');
          }
          const cs = vmMod.runInContext('typeof __csLog!=="undefined"?__csLog:[]', ctx);
          if (cs && cs.length) {
            require('fs').writeFileSync('cs_log.json', JSON.stringify(cs), 'utf-8');
            log('[ANS-DUMP] cs 常量表 ' + cs.length + ' 条已写出');
          }
          const c3 = vmMod.runInContext('typeof __call3!=="undefined"?__call3:[]', ctx);
          if (c3 && c3.length) {
            require('fs').writeFileSync('call3.json', JSON.stringify(c3), 'utf-8');
            log('[ANS-DUMP] op3 调用日志 ' + c3.length + ' 条已写出');
          }
          const c5 = vmMod.runInContext('typeof __call5!=="undefined"?__call5:[]', ctx);
          if (c5 && c5.length) {
            require('fs').writeFileSync('call5.json', JSON.stringify(c5), 'utf-8');
            log('[ANS-DUMP] op5 调用日志 ' + c5.length + ' 条已写出');
          }
        } catch (e) { log('[ANS-DUMP] env 写出 err: ' + e.message); }
      } catch (e) { log('[ANS-DUMP] exit 写出 err: ' + e.message); }
    }
  ...
  const check = setInterval(() => {
    const c = getCookie2();
    // TEMP-PURE: 收集解释器返回 dump (真实 ans)
    if (captureInner) {
      try {
        const d = vmMod.runInContext('typeof __ansDump!=="undefined"?__ansDump:[]', ctx);
        if (d && d.length) {
          require('fs').writeFileSync('real_ans_dump.json', JSON.stringify(d), 'utf-8');
          if (d.length > (globalThis.__lastAnsLen || 0)) {
            globalThis.__lastAnsLen = d.length;
            log('[ANS-DUMP] ' + d.length + ' 条, tail: ' + String(d[d.length - 1]).slice(0, 100));
          }
        }
      } catch (e) {}
    }
    // TEMP-PURE: cookie 已生成 (内层已执行) 后 dump 真实 $_ts + 全部 _$ 全局
    if (!globalThis.__tsDumped && c.length >= 100) {
      globalThis.__tsDumped = true;
      try {
        const ts = vmMod.runInContext('$_ts', ctx);
        const globs = vmMod.runInContext(
          '(function(){var out={};var ks=Object.keys(this);for(var i=0;i<ks.length;i++){'
          + 'var k=ks[i];if(k.indexOf("_$")===0){var v=this[k];var t=typeof v;'
          + 'out[k]=t;}}return out;})()', ctx);
        require('fs').writeFileSync('real_ts_dump.json', JSON.stringify({
          nsd: ts && ts.nsd, cdLen: ts && ts.cd && ts.cd.length,
          scj: ts && ts.scj, aebi: ts && ts.aebi, cp: ts && ts.cp, jf: ts && ts.jf,
          globs: globs,
        }), 'utf-8');
        log('[REAL-TS] mid-run dump saved');
      } catch (e) { log('[REAL-TS] err: ' + e.message); }
    }
    if (evalCaps.length) {
